...
class PracticalQuantumSystem:
    ...
    def calculate_coherence_time(self) -> float:
        ...
        T_coherence_0K = 1e-3  # 1 ms
        
        # Temperature-dependent decoherence
        # Using empirical formula for superconducting qubits
        if self.temperature < 1:
            # mK range - quantum limit
            T_coherence = T_coherence_0K * np.exp(-self.temperature / 0.1)
        elif self.temperature < 10:
            # He temperature - good coherence
            T_coherence = T_coherence_0K * 0.1 * np.exp(-self.temperature / 5)
        elif self.temperature < 100:
            # N2 temperature - moderate coherence  
            T_coherence = T_coherence_0K * 0.01 * np.exp(-self.temperature / 50)
        else:
            # Room temperature - poor coherence
            T_coherence = 1e-9  # 1 ns
        
        # Additional decoherence from equipment
        T_equipment = 1 / (self.equipment.laser_linewidth_MHz * 1e6)
        
        # Combined coherence time (ensure non-zero)
        T_combined = 1 / (1/T_coherence + 1/T_equipment)
        
        # Ensure minimum coherence time for numerical stability
        return max(T_combined, 1e-9)
    ...
    def add_realistic_noise(self, signal: np.ndarray, t: np.ndarray) -> np.ndarray:
        ...
        # Scale noise to be more realistic (not overwhelming the signal)
        noise_scale = 0.1  # 10% noise level
        
        # 1. Thermal noise (Gaussian)
        thermal = np.random.randn(len(signal)) * self.thermal_noise_V * noise_scale
        ...
        pink_noise = pink_noise * self.flicker_noise_coefficient * noise_scale
...
def run_temperature_comparison():
    ...
    temperatures = [4.2, 77, 295]  # Helium, Nitrogen, Room temp
    ...
    for T in temperatures:
        ...
        for state in state_types:
            ...
            results = system.run_practical_experiment(
                duration_s=1e-4,  # 100 microseconds
                state_type=state
            )
...
if __name__ == "__main__":
    ...
    # Run comparison at different temperatures
